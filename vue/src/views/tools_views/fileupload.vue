...
export default {
  data() {
    return {
      ...
      file:'',
      formData:'',
      actionUrl:'http://localhost:9090/upload/file',
  ...
  methods: {
    ...
    submit() {
      ...
      // 白名单，前端验证可绕过
      let fileType = ['jpg','png','gif','doc','pdf','exe','docx','html','jpeg','mp4']
      if (index != -1) {
        let suffix = fileName.substr(index + 1)
            // .toLowerCase();
        if (fileType.includes(suffix)) {
          request.post('/upload/file', file, {headers: {'Content-Type': 'multipart/form-data'}}
          ).then(res => {
          ...
          this.$message.error("文件格式错误！请选择'jpg','png','gif','doc','pdf','exe','docx','html','jpeg','mp4'格式的文件")
        }
      }
